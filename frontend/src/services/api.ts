...
class ApiService {
  ...
  async login(username: string, password: string) {
    const response = await this.client.post('/users/users/login/', { username, password })
    return response.data
  }

  async register(data: any) {
    const response = await this.client.post('/users/users/register/', data)
    return response.data
  }

  async logout() {
    const response = await this.client.post('/users/users/logout/')
    return response.data
  }
  ...
  async getExercises(params?: { subject?: string; level?: string; difficulty?: string }) {
    const response = await this.client.get('/exercises/exercises/', { params })
    return response.data
  }

  async getExercise(id: number) {
    const response = await this.client.get(`/exercises/exercises/${id}/`)
    return response.data
  }

  async createExercise(data: any) {
    const response = await this.client.post('/exercises/exercises/', data)
    return response.data
  }

  async submitExercise(id: number, data: { answer: any; time_spent?: number; hints_used?: number }) {
    const response = await this.client.post(`/exercises/exercises/${id}/submit/`, data)
    return response.data
  }

  async getMyExerciseAttempts() {
    const response = await this.client.get('/exercises/exercises/my_attempts/')
    return response.data
  }
